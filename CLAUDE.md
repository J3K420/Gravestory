...
- **Stake a FourThought prediction at the start of substantive work, reflect on it after.** Before building, run `node tools/fourthought.js predict "<what this session should achieve>" --valence N(1-10) --certainty C(0-100, higher=more sure, default 100) [--branch B] [--tag X]` (alias `4t predict ...`). After it ships, `4t reflect <id> --outcome shipped|partial|failed|abandoned --worth N` — the tool auto-joins token spend + git activity and computes ROI/calibration. This is the ROI-tracking ledger (FourThought dialectic by speakerjohnash; certainty scale matches his source code, NOT an inverted 0-1); the discipline of staking BEFORE the outcome is the whole point. Reports: `4t report calibration|roi|forecast|levers`. Skip for trivial one-off edits.
- **Mobile JS changes ship via OTA to the `production` channel** (where live testers are): `npx eas update --branch production --environment production --platform android`. Verify a clean worktree and the production channel before publishing. Native-module changes need a new build, not an OTA.
...
## Web landing-page direction (EXECUTED)

The web app is a **thin landing page** whose job is to point visitors to the native apps on the **Google Play Store (Android)** and the **Apple App Store (iOS)**. The interactive scan/research pipeline is **mobile-only going forward**. The former web PWA scan pipeline was removed — do not recreate it or keep it at parity with mobile.
...
- **Status:** the landing-page conversion was completed on 2026-06-29. The canonical public host is `https://gravestory.pages.dev/`; deployment and retirement gates are documented in `docs/cloudflare-pages-cutover.md`. When building, default to mobile-only for pipeline work without re-asking.
...
- Current Shared Remembrance tester build: preview APK `https://expo.dev/artifacts/eas/jliSAPelRZW4Gva-QpUsaqZs-i7HVAk5qEKyH8PVOpU.apk` (versionCode 17, runtime `1.1.0`, channel `preview`). Preview OTA command: from `mobile/`, after verifying clean pushed source, run `npx eas update --channel preview --environment production --platform android`; never substitute the production channel without explicit approval.
...
- **LIVE on the Google Play Store** (Android, versionCode 15 — owner-confirmed 2026-07-13). The current internal preview APK is versionCode 17/runtime `1.1.0`; vc17 is not live on Google Play. Production access approved; managed publishing off.
- **Shared Remembrance repair verified on preview (owner-confirmed 2026-08-07):** migration 038 was manually applied to production, restoring remembrance photo uploads. Mobile commit `9a267a0` normalizes nullable story arrays at the Result rendering boundary and in `rowToStory()`, so newly saved and sync-recovered private/pending/public remembrances open without the `sources.length` ErrorBoundary crash; real source/link arrays and private-photo bearer headers are preserved. Android preview update group `07e7eef9-6c71-45ac-8447-a72e079f6ff3` published that commit to runtime `1.1.0`, and the owner confirmed it works. The production mobile channel was untouched. Worker hardening included in commit `86845c0` remains **not deployed**; do not deploy it or make another production change without explicit approval.
- **Next production AAB deliberately deferred (owner decision 2026-08-07):** the live Play build is versionCode 15/runtime `exposdk:54.0.0`, so it cannot receive the runtime `1.1.0` Shared Remembrance preview OTA. Do not create or submit a production AAB, and do not publish a production OTA, yet. In a new session, evaluate and integrate Tavily alternatives to reduce per-scan search cost; provider and architecture are not decided. Validate that work, confirm the next unused Play versionCode, then build one production AAB containing both the Shared Remembrance repair and the search-cost changes.
- **Web is now a landing page** (executed 2026-06-29; canonical host migrated 2026-07-13): `https://gravestory.pages.dev/` serves a homepage (Google Play button + greyed "iOS — coming soon" + a link to the community global map + footer legal links), the **community global map**, and a **read-only public-bio view** (tap a pin → popup → read-only bio, kept as the SEO surface). The entire scan/research/auth-write pipeline was DELETED from web (`js/` lost ~28 files); surviving web JS is config/util-json/util-html/util-dom/auth(read-only client)/symbols/grave-markers/render-result(read-only)/map-global/analytics/api-reports. Worker + Supabase remain because mobile depends on them; web reads public story/photo data and permits only the narrow anonymous `content_reports` and `analytics_events` inserts. No sign-in on web; every visitor gets the full 500-row map. Service-worker cache `gravestory-v69` is live and verified. Keep the legacy GitHub Pages site available until the production OTA and Google Play cutover are verified.
