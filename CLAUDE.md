...
- **Two-pass Overpass grave-node search (web only)** — Pass 1 searches tagged nodes (historic=memorial/tomb/grave/monument/mausoleum, tourism=attraction, cemetery=grave, memorial=*, building=tomb/mausoleum) within 1000m. Pass 2, if pass 1 misses, searches any named node within the Nominatim bounding box (requires 100% token match to control false positives on untagged nodes). Famous graves often use `tourism=attraction`, not `historic=grave`. **On mobile this search is non-functional** — Overpass mirrors block Cloudflare Worker IPs (see boundary polygon note). The Worker's `/overpass` endpoint exists but returns 502; `api-nominatim.js` catches this and falls back to cemetery-center coordinates. Pin accuracy improvement for mobile is a TODO (see Phase 9 scope).

- **Cemetery boundary polygon (web only)** — `fetchOSMCemeteryBoundary` in `map-cemetery.js` queries Overpass for ways and relations within 1000m. Relations need `stitchOuterRing()` to order member ways correctly (raw concatenation produces crossed lines). Scoring: name-match first (prevents dense complexes like Cypress Hills, Queens from overriding the specific cemetery), then relation over way, then smallest area. Relations stitching to >2000 points are skipped (district-level, not cemetery-level). The `cemeteryName` (first comma-segment of location string) is threaded from `initCemeteryMap` → `renderLeafletMap` → `loadAndDrawBoundary` → `fetchOSMCemeteryBoundary`. Clear `gs_grave_cache` from localStorage to force a fresh Overpass lookup. **Mobile does not draw a boundary polygon** — all public Overpass mirrors (overpass-api.de, kumi.systems, maps.mail.ru) block Cloudflare Worker IPs with 403, and direct calls from React Native's HTTP stack get 406. Do not attempt to use Overpass from mobile.
...
- **Search accuracy improvements (Session 9)** — layered improvements now live in both `js/` (web) and `mobile/src/lib/` (mobile):
  ...
  - *Nickname/abbreviation expansion* — `EXPAND` table (~60 entries) in `api-tavily.js` and `api-wikitree.js` maps period abbreviations and informal names (Wm→William, Geo→George, Lizzie→Elizabeth, etc.) to formal forms. Tavily fires queries for both forms; WikiTree's `firstNamesMatch()` is nickname-aware and adds a third search pass with the expanded first name when the abbreviated form returns nothing.
  ...
  - *Multi-stone detection* — when `graveData.multiple_subjects === true`, the pipeline shows a warning (web: loading-step text; mobile: Alert) that the bio will focus on the primary inscription and the user should photograph each stone separately.
  ...
  - *Citation integrity* — `_validateCitations()` in `biography.js` strips any `[N]` markers whose index exceeds the sources array after generation, preventing orphan citations that appear authoritative but point to nothing.
  - *Namesake collision guard* — the historical-figure exception in `biography.js` now requires BOTH date alignment (±5 years) AND that the Wikipedia article (if found) confirms the same person before invoking full-biography mode. Prevents "John Adams d.1931" inheriting the Founding Father's biography.
...
mobile/
  ...
  src/
    lib/
      ...
      api-gemini.js             — verifyIsGravestone, readGravestone (ES module). readGravestone returns name_confidence (high/medium/low), alternate_names (1-2 alternate spellings when stone is weathered/ambiguous), and multiple_subjects (true when photo shows multiple separate distinct stones) in addition to standard fields.
...
- **Phase 8** ✅ — Full visual design overhaul: theme.js design system (Fraunces + Hanken Grotesk, new palette), Icons.js SVG set, all screens redesigned. Per-user storage isolation (user-scoped AsyncStorage keys). forwardGeocode multi-query, geographic context filter, grave-cache.js. Custom SVG gravestone map marker. Pull-to-refresh on all screens. Bug fixes: CemeteryMapScreen userId, syncOnSignIn full-pull-on-empty. Note: two-pass Overpass grave-node search ported but non-functional on mobile (Overpass blocks Cloudflare IPs).
- **Phase 9** 🔲 — Play Store submission prep, OTA updates (EAS Update), payments (RevenueCat + Google Play Billing), iOS TestFlight build. Also: improve mobile gravestone pin accuracy without Overpass (Overpass is fully blocked on mobile — do not use it; find a Nominatim-based or alternative approach). Remove leftover `/overpass` Worker endpoint and revert `api-nominatim.js` Overpass calls once pin accuracy is addressed.
