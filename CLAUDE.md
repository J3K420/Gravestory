...
js/
  ...
  api-gemini.js          — geminiCallWithFallback, verifyIsGravestone, readGravestone. readGravestone emits name_confidence (high/medium/low), alternate_names (1-2 alternate spellings when weathered), multiple_subjects (true ONLY when the photo contains physically separate freestanding grave markers at distinct locations — NOT a single shared family stone listing multiple people; if uncertain, false), specific symbol names, a `subjects` array (one entry per EVERY DECEASED person visible anywhere in the photo regardless of multiple_subjects — "on this stone" was the old wording and caused regressions; aliases of one person merge to a single entry; living relatives excluded), `maiden_name` (deceased's birth surname from "née Brown" etc. — feeds the WikiTree maiden-name pass for married women), and `relationships` array ([{relation, name}] for spouse/parents — feeds WikiTree relationship scoring). This subjects array drives the per-person multi-subject biography path.
  api-nominatim.js       — reverseGeocode (GPS coords → "City, State") + reverseGeocodeCemetery (GPS → enclosing cemetery name via high-zoom reverse lookup; threaded into searchForPerson as a Tavily disambiguator) + forwardGeocode (Nominatim + Overpass named-grave search). reverseGeocode is already called in the pipeline before readGravestone.
  api-tavily.js          — searchForPerson(graveData, location, cemeteryName). cemeteryName (from reverseGeocodeCemetery) is injected into the FindAGrave + obituary slots as an exact-phrase disambiguator for common names. Slots 5/6 historical-obituary boundary is 1928 (matches the Chronicling America cutoff — keep in sync). Contains: _EXPAND nickname table (~60 entries), _expandName(), _parseAgeAtDeath() (derives missing year from "aged N yrs" inscription), _SYMBOL_QUERIES map (GAR/Masonic/Odd Fellows/military/VFW → targeted record repos). Capped at 6 queries, max_results: 2, session-level _searchCache.
  api-wikidata.js        — queryWikidata(name, deathYear). Free Wikidata, no proxy (CORS-open). ALIAS-AWARE: uses action=wbsearchentities (matches maiden/birth/stage names + near-spellings, not just exact rdfs:label), fetches P31=human candidates in one SPARQL VALUES query, then scores by death-year proximity (rejects candidates >5yr off — prevents namesake picks) instead of blindly taking bindings[0]. Returns { birthDate, deathDate, burialPlaceLabel, burialEntityId, burialCoords, wikipediaTitle } or null. wikipediaTitle is the en.wikipedia sitelink (exposed for future article-bridging when the stone name differs from the article title; not yet consumed). Only fires when name_confidence === 'high'. burialCoords used as GPS fallback when no EXIF/device GPS was captured.
  api-chroniclingamerica.js — searchChroniclingAmerica(name, deathYear). Uses the chroniclingamerica.loc.gov/search/pages/results endpoint (searches OCR'd page TEXT, returns ocr_eng snippets windowed around the surname — much richer than the old loc.gov/collections metadata endpoint). date1/date2 window the search to deathYear ± 1. Cutoff is 1928 (returns [] for deathYear > 1928) — the source_type: 'public_domain' tag stays honest only under the US rolling PD wall (works published ≤1929 PD in 2026). Runs in parallel with Tavily; results merged into searchResults before generateBiography. Corroboration in biography.js uses CA content only for name-presence matching, never date parsing — so noisy OCR cannot fabricate a date conflict.
  api-wikitree.js        — searchWikiTree(graveData, location). Passes: date-filtered → maiden-name (when graveData.maiden_name present — married women are indexed under birth surname) → unfiltered → expanded-first-name fallback. Nickname-aware _wtFirstNamesMatch(). Geographic alignment scoring: _wtExtractUSState() + ±30/−20 on burial-state match/mismatch. Relationship scoring: candidates' Father/Mother/Spouses fields (already fetched) are token-matched against graveData.relationships — spouse hit +40, parent hit +25 (additive only, never rejects). Last-name match accepts married surname, birth surname, OR the stone's maiden_name.
