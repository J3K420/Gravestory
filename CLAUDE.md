...
- **Search Rec 2 (FamilySearch) — pending API registration**: the `search-improvement-recommendations.md` document describes integrating the FamilySearch Person Search API (free, 8B+ records, better US/immigrant family coverage than WikiTree). Requires creating a free developer account at `familysearch.org/developers`, registering an app to get a `client_id`, and storing the secret in the Cloudflare Worker. Implementation: new `api-familysearch.js` modules (web + mobile), `unauthenticated_session` token fetched once per session and cached, results mapped to the same shape as WikiTree and added to `buildCorroborationSummary`. Enables a Tavily early-exit optimisation (skip slots 4+6 when both WikiTree and FamilySearch return aligned matches).

- **Search Rec 6 (biography result cache) — needs Supabase RPC**: the cache would let repeat scans of any known grave (by any signed-in user) skip Tavily + WikiTree + Gemini entirely and serve the cached result. Requires a new `find_grave` Supabase RPC (read-only version of `find_or_create_grave`) so the pipeline can look up a `grave_id` by name + GPS proximity *before* firing the expensive search steps. Once that RPC exists: after `readGravestone`, call `find_grave(primaryName, lat, lng)` → if hit, query `stories` table for a recent public story keyed to that `grave_id` → use it directly and skip to `forwardGeocode`. Cache key: name (normalised) + deathYear + `grave_id`. 90-day TTL is sufficient — biography content rarely changes. Guest users always run the full pipeline (no `grave_id` without sign-in).
