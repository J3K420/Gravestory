...
- **Grave-node search — web vs mobile** — Web uses a two-pass Overpass query: Pass 1 searches tagged nodes (historic=memorial/tomb/grave/monument/mausoleum, tourism=attraction, cemetery=grave, memorial=*, building=tomb/mausoleum) within 1000m; Pass 2, if pass 1 misses, searches any named node within the Nominatim bounding box (100% token match required). Famous graves often use `tourism=attraction`, not `historic=grave`. Mobile cannot use Overpass (all mirrors return 403/406 to Cloudflare Worker IPs and to React Native's HTTP stack directly). Mobile uses a **Nominatim viewbox search** instead: after finding the cemetery center, `forwardGeocode` calls `GET /search?q={personName}&viewbox={bbox+0.001pad}&bounded=1&limit=10`, scores results by name-token match, and returns the best match above threshold. Falls back to cemetery-center coordinates when no match is found.
...
- **Phase 8b** ✅ — Mobile pin accuracy: replaced non-functional Overpass grave-node search with Nominatim viewbox search (`bounded=1` within cemetery bbox). Removed `PROXY_BASE` import and dead `OVERPASS` constant from `mobile/src/lib/api-nominatim.js`.
