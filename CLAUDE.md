...
- **Phase 8f** 🔲 — Web parity for Phase 8e features: (1) `js/persistence.js` — add `grave_id` + `source` to `storyToRow`/`rowToStory`; (2) `index.html` pipeline — track `source: 'camera'/'library'` on story, call `find_or_create_grave` RPC after biography; (3) `js/map-global.js` — client-side dedup by `grave_id` then ~20 m GPS cell (same logic as mobile `GlobalMapScreen`); (4) new `js/api-tributes.js` — vanilla JS port of `getTributes`/`setTribute` using `supabaseClient`; (5) `js/render-result.js` — tribute counts always visible when `grave_id` present, candle/flower buttons for camera-sourced stories only.
