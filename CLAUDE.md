...
- **LIVE on the Google Play Store** (Android, versionCode 15 — owner-confirmed 2026-07-13). `mobile/app.config.js` currently reserves versionCode 16 for a future AAB, but vc16 is not live. Production access approved; managed publishing off.
- **Web is now a landing page** (executed 2026-06-29; canonical host migrated 2026-07-13): `https://gravestory.pages.dev/` serves a homepage (Google Play button + greyed "iOS — coming soon" + a link to the community global map + footer legal links), the **community global map**, and a **read-only public-bio view** (tap a pin → popup → read-only bio, kept as the SEO surface). The entire scan/research/auth-write pipeline was DELETED from web (`js/` lost ~28 files); surviving web JS is config/util-json/util-html/util-dom/auth(read-only client)/symbols/grave-markers/render-result(read-only)/map-global/analytics/api-reports. Worker + Supabase remain because mobile depends on them; web reads public story/photo data and permits only the narrow anonymous `content_reports` and `analytics_events` inserts. No sign-in on web; every visitor gets the full 500-row map. Service-worker source cache is `gravestory-v69` (live Pages remains v68 until the reviewed bundle is redeployed). Keep the legacy GitHub Pages site available until the production OTA and Google Play cutover are verified.
