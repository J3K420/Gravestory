...
js/
  ...
  api-gemini.js          — geminiCallWithFallback, verifyIsGravestone, readGravestone. readGravestone emits name_confidence (high/medium/low), alternate_names (1-2 alternate spellings when weathered), multiple_subjects (true when photo shows separate distinct stones), specific symbol names, and a `subjects` array (one entry per DECEASED person with their OWN name/birth_date/death_date — drives the per-person multi-subject biography path; aliases of one person merge to a single entry; living relatives excluded).
