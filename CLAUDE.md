...
- **Search accuracy improvements (Session 9)** — layered improvements now live in both `js/` (web) and `mobile/src/lib/` (mobile):
  ...
  - *OCR alternate readings* — `readGravestone` emits `name_confidence` (high/medium/low), `alternate_names` (1-2 plausible alternate spellings), and `multiple_subjects` (true when multiple separate stones are in frame). Alternate readings feed into Tavily query variants when confidence is not high.
  ...
  - *Citation integrity* — `_validateCitations()` in `biography.js` strips any `[N]` markers whose index exceeds the sources array after generation, preventing orphan citations.
  - *Namesake collision guard* — the historical-figure exception in `biography.js` requires graveData birth/death dates to be consistent (±5 years) with the famous figure's actual dates before invoking full-biography mode. Prevents "John Adams d.1931" inheriting the Founding Father's biography.
