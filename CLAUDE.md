...
- **Cemetery boundary polygon** — Web: `fetchOSMCemeteryBoundary` in `map-cemetery.js` queries Overpass for ways and relations within 1000m. Relations need `stitchOuterRing()` to order member ways correctly. Scoring: name-match first, then relation over way, then smallest area. Relations stitching to >2000 points are skipped. The `cemeteryName` is threaded from `initCemeteryMap` → `renderLeafletMap` → `loadAndDrawBoundary` → `fetchOSMCemeteryBoundary`. Mobile: `fetchOSMCemeteryBoundary` in `CemeteryMapScreen.js` uses Nominatim `/search?q={cemeteryName}&polygon_geojson=1` instead of Overpass (Overpass is blocked on mobile). Filters results to those with non-Point geometry within ~0.15° of the grave coords, prefers `landuse=cemetery` / `amenity=grave_yard`, returns the best polygon via `geojsonToCoords`. Do not attempt to use Overpass from mobile.
...
- **Phase 8b** ✅ — Mobile pin accuracy: replaced non-functional Overpass grave-node search with two-pass Nominatim+Photon approach. Nominatim uses viewbox bias without `bounded=1` (avoids importance-score cutoff) + bbox proximity filter. Photon (Elasticsearch-backed) as fallback for low-importance grave nodes. Camera/EXIF GPS always takes priority over geocoded coords. Corrected CLAUDE.md: mobile boundary polygon IS drawn via Nominatim `polygon_geojson=1`, not Overpass.
