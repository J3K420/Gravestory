...
const _CEMETERY_TYPE_QIDS = new Set([
  'Q39614',    // cemetery
  'Q19844914', // burial ground / graveyard
...
// the surname/person gates downstream cannot see. [review M6]
async function _resolveCemeteryEntity(cemeteryName, userGps) {
  if (!cemeteryName || cemeteryName.trim().length < 3) return null;
  const label = cemeteryName.split(',')[0].trim();
  const ids = await _wbSearchEntities(label, 5);
  ...
  for (const id of ids) {
    ...
    if (!types.some(t => _CEMETERY_TYPE_QIDS.has(t))) continue;
...
// name matches — or null. Recovers famous graves identified by a bare surname
// banner (e.g. "BOOTH" at Green Mount Cemetery → John Wilkes Booth) that the
// name-first paths (WikiTree needs two tokens; Wikidata-by-name has no death year
// to disambiguate) cannot resolve.
//
...
//     (Green Mount has several Booths) must not be guessed.
//   - The caller must still corroborate the candidate against an independent
//     source (Wikipedia) before naming them in a biography.
//
// Returns { name, birthDate, deathDate, burialPlaceLabel, burialEntityId,
//   burialCoords, wikipediaTitle, entityId, _viaBurialPlace } or null.
async function queryWikidataByBurialPlace(cemeteryName, surname, opts) {
  ...
  const cemeteryQ = await _resolveCemeteryEntity(cemeteryName, _userGps);
