...
// AND some first-name token must align (nickname-aware via _wtFirstNamesMatch) —
...
function _wtRelationNameMatch(stoneName, candidateName) {
  ...
  if (a.length === 1) return b.some(tb => tb === a[0] || _wtFirstNamesMatch(a[0], tb));
  ...
  const firstHit = a.slice(0, -1).some(ta => b.slice(0, -1).some(tb => _wtFirstNamesMatch(ta, tb)));
  ...
  try {
    ...
    for (const m of matches) {
      ...
      if (hasKnownRelatives) {
        ...
        // _wtRelationNameMatch. Father/Mother are bare person keys (not names),
        ...
        if (candidateNames.length) {
          const spouseHit = knownRelatives.spouse.some(rn =>
            candidateNames.some(cn => _wtRelationNameMatch(rn, cn)));
