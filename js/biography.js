...
function _buildBurialCandidatePartial(burialCandidate, location) {
  ...
  const yrs = [burialCandidate.birthDate && burialCandidate.birthDate.slice(0, 4),
    burialCandidate.deathDate && burialCandidate.deathDate.slice(0, 4)]
    .filter(Boolean).join('–');
...
async function generateBiography(graveData, searchResults, wikiData, location, wikipediaSummary, wikidataResult, burialCandidate) {
  ...
  if (!hasRealSources) {
    ...
    // unconfirmed → no-name result (_buildBurialCandidatePartial returns null), i.e.
    ...
    const _bcSrc = _buildBurialCandidatePartial(burialCandidate, location);
    if (_bcSrc) {
      return {
