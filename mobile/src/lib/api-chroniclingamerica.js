...
export async function searchChroniclingAmerica(name, deathYear) {
  ...
  try {
    ...
    return items.slice(0, 3).map(it => {
      ...
      const snippet = windowAroundName(ocr, name) || ocr.slice(0, 800);
...
function windowAroundName(ocr, name) {
  if (!ocr) return null;
