...
NON-LATIN / CJK INSCRIPTIONS (Chinese, Japanese, Korean, and other non-English stones):
The rules above use English exemplars, but they apply in EVERY language. When the stone is inscribed in Chinese (or another non-Latin script), read and interpret it directly — do not skip it, and do not merely transliterate without meaning.
- READING ORDER: Traditional Chinese/Japanese stones are read in vertical columns, TOP-TO-BOTTOM and RIGHT-TO-LEFT. The canonical (but NOT universal — modern and horizontal layouts exist) three-column form is: the RIGHT column often gives the ancestral origin (province/county/village), the CENTER column the deceased's name framed by honorifics, and the LEFT column the descendants and/or the date. Use position as a hint, not a rule: a short place-like string in the right column is commonly an ancestral home rather than a person's name — but if it reads as a personal name, treat it as one. When unsure whether a right-column string is a place or a name, put it in "notes" rather than forcing it into "ancestral_origin".
- PRESERVE THE GLYPH: transcribe the actual character as carved (traditional/variant form), do NOT "correct" it to a simplified form. Put the verbatim characters in "inscription".
- NAME ORDER: Chinese/Korean/Japanese names are SURNAME-FIRST. The first character(s) are the family name; the rest is the given name. Set family_name to the surname characters when the stone clearly gives the deceased's own surname.
- ONE PERSON, TWO NAMES: A US Chinese/Asian-American stone often shows BOTH the native-script name AND a romanized/anglicized English name for the SAME person. Reconcile them into ONE subject (merge, as with any alias) — do NOT treat the two names as two people and do NOT set multiple_subjects for this.
- KINSHIP HONORIFICS — distinguish two kinds, exactly as with the English rules above:
    · TITLES ON THE DECEASED mark WHO IS BURIED here, they do NOT make the deceased a mere relative: 顯考/先考 ("[our] late father") and 顯妣/先妣 ("[our] late mother") mean the DECEASED is the descendants' late father/mother (like an English "Our Beloved Father"); 之墓/之墳 ("tomb of [Name]") likewise marks [Name] as the DECEASED. The name these frame is the deceased — set it as primary_name, do not demote it to a relative.
    · RELATIONAL GOVERNORS name a RELATIVE, not the deceased — e.g. …之妻 / …妻 ("wife of …"), …之夫 ("husband of …"), …子/…女 ("son/daughter of …"): the OTHER name they point to is the relative. Record these in "relationships" (spouse/father/mother/son/daughter) just as you do for the English "wife of / son of" phrases.
- MAIDEN NAME: on a Chinese stone a married woman is often shown as "[husband-surname] 門 [birth-surname] 氏". The character 氏 follows her BIRTH (natal) surname and 門 marks the family she married into — put the BIRTH surname (the one before 氏) in "maiden_name" (the Chinese equivalent of "née"). Caveat: 氏 can also simply mark "a woman of the ___ clan" — if there is no separate married surname shown, that surname may be her own primary surname, so use judgement and prefer family_name when only one surname is present.
- DATES: normalize a reign-era / Republic date (民國 N年 = 1911 + N; e.g. 民國38年 = 1949) to a Western year (YYYY) in birth_date/death_date only when you can derive it confidently; keep the original date text verbatim in "inscription". 享壽/享年 N marks age-at-death. DO NOT guess a Western year for a bare sexagenary (干支) or lunar date that lacks an anchoring reign-era or Western year — a 60-year-cycle date is ambiguous without a century, so leave birth_date/death_date empty and keep the original in "inscription" rather than fabricating a year.
- ROMANIZATION FOR RECORDS: when you provide a romanized form of a Chinese name or ancestral village, prefer the CANTONESE / Sze Yup (Toishanese) spelling that would appear on early-1900s US immigration and cemetery records (e.g. Chan/Chin, Wong, Lee, Toishan/Hoisan), NOT Mandarin Pinyin (Chen, Huang, Li, Taishan) — the overwhelming majority of historical Chinese-Americans trace to Taishan/Sze Yup and their US paper trail uses those ad-hoc spellings. Offer plausible variant spellings in "alternate_names". Treat any romanization as a best-guess, not a certainty.
- ANCESTRAL ORIGIN: put the province/county/village of origin (the right-column place) in "ancestral_origin" — it is the single most valuable genealogical datum on a Chinese stone; do NOT mistake it for the burial location or the deceased's surname.
...
Return ONLY a valid JSON object with these exact fields:
{
  ...
  "symbols": ["list of symbols, emblems, or decorations — be specific: 'GAR Grand Army of the Republic emblem', 'Masonic square and compass', 'Odd Fellows three links', etc. For a non-Latin/CJK symbol, give an ENGLISH descriptor AND ALWAYS append the actual character in parentheses so the meaning can be looked up — e.g. 'Fu character (福) — fortune/blessing', 'Shou character (壽) — longevity', 'dragon (龍)', 'fenghuang (鳳凰)', 'double happiness (囍)', 'manji (卍)'. Always include the parenthesised glyph for a CJK symbol, never the English word alone. For the Chinese phoenix use 'fenghuang (鳳凰)' — do NOT write the bare word 'phoenix', which denotes the unrelated Western resurrection phoenix."],
  ...
  "maiden_name": "the deceased's birth/maiden surname if the inscription reveals it — e.g. 'née Brown', 'born Smith', a married woman shown as 'Mary Jones, daughter of the Brown family', or the surname before 氏/門 on a Chinese stone. Genealogy records index married women under their maiden name, so capture it whenever shown. Empty/null if not indicated.",
  "ancestral_origin": "for a stone that records a place of family origin (common on Chinese and other immigrant stones — e.g. the right-column province/county/village such as '新寧縣' Taishan/Sunning, or 'County Cork, Ireland'), capture it here as written, with a record-style romanization if non-Latin. This is the ancestral HOMELAND, NOT the burial location. Empty/null if not shown.",
...
If multiple deceased people share the stone, use the names array and pick the most prominent as primary_name, and list every deceased person with their own dates in subjects. Return only JSON.`;
  ...
  return safeParseJSON(text, {
    ...
    inscription: '', symbols: [], family_name: '', maiden_name: '', ancestral_origin: '', relationships: [], notes: '',
